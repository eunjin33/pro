...
public class MemoDAO {
    ...
    public static ArrayList<MemoVO> selectAll(DBHelper dbHelper) {
        ...
        String sql = "select title, content, img from diary order by title desc";
        Cursor cursor = db.rawQuery(sql, null);
        while (cursor.moveToNext()){
            MemoVO memoVO = new MemoVO();
            memoVO.setTitle(cursor.getString(0));
            memoVO.setContent(cursor.getString(1));
            memoVO.setImg(cursor.getString(2));
            list.add(memoVO);
        }
        db.close();
        return list;
...
//
//        if(memoVO.getImg() != null){
//            contentValues.put("img", memoVO.getImg());
//        }
//
